...
namespace Solana.Unity.SDK
{
    ...
    public class Web3 : MonoBehaviour
    {
        ...
        {
            #if UNITY_EDITOR
            MockLoginForTesting(); // Use mock login in Editor
            #else
            LoginWalletAdapter().AsUniTask().Forget(); // Use real login in builds
            #endif
        }
        ...
        #if UNITY_EDITOR
        public void MockLoginForTesting()
        {
            // Create a mock Account directly
            var mockAccount = new Account(); // Generates a new random key pair

            // Trigger the OnLogin event immediately without actual login
            OnLogin?.Invoke(mockAccount);
            Debug.Log("Mock login set successfully with account: " + mockAccount.PublicKey);
        }
        #endif
